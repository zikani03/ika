...
function __ikaSetInputValue(el, fakerSpecOrFn, fakerObj) {
    ...
    if (typeof (value) === 'function') {
        el.setAttribute("value", value(el, faker) || '');
    } else {
...
function __ikahandleSubmit(evt: Event) {
    ...
    const catchAllFormExists: boolean = window.ikaConfig.forms['*'] ? true : false,
    ...
    if (noConfiguredFormsFoundOnPage && catchAllFormExists && rawText.length < 1) {
